...
namespace Templates\Phoundation\AdminLteV4\Html\Components\Widgets\Menus;
...
class TemplateTopMenu extends TemplateRenderer
{
    ...
    /**
     * Render the HTML for the AdminLteV4 top menu
     *
     * @return string|null
     */
    public function render(): ?string
    {
        $return = '<ul class="navbar-nav">
                        <li class="nav-item">
                            ' . Anchor::new()->setClass('nav-link')->addData('pushmenu', 'widget')->setContent('<i class="fas fa-bars"></i>') . '
                        </li>
                        <li class="nav-item d-none d-sm-inline-block">
                            ' . Anchor::new()->setClass(Url::newCurrent())->setClass('nav-link')->setContent(tr('Home')) . '
                        </li>';
        ...
        if ($this->_component->getSource()) {
            foreach ($this->_component->getSource() as $label => $entry) {
                ...
                $return .= '<li class="nav-item d-none d-sm-inline-block">
                                ' . Anchor::new($entry['url'])->setClass('nav-link')->setContent($label) . '
                            </li>';
            }
        }
